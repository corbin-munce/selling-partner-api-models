...
namespace Amazon.SellingPartnerAPIAA
{
    public class AWSSignerHelper
    {
        ...
        public virtual DateTime InitializeHeaders(RestRequest restRequest, string host)
        {
            ...
            {
                if ((parameter.Name == XAmzDateHeaderName || parameter.Name == HostHeaderName) && ParameterType.HttpHeader.Equals(parameter.Type))
                {
                    removes.Add(parameter.Name);
                }
            }
            foreach (string remove in removes)
            {
                restRequest.Parameters.RemoveParameter(remove);
            }
